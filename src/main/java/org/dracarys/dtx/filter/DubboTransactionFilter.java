 ...
 * 3.客户端需要传递事务时，声明dubbo服务引用需要添加过滤器 例如：<xmp> <dubbo:reference id="testService1" interface="test.TestService" protocol="dubbo" filter="DubboTx" /> </xmp> 4.如果需要全局传递事务，可进行如下配置：<xmp>
 * <dubbo:consumer filter="DubboTx" retries="0" /> </xmp> <b>建议不要启用这个配置，dubbo调用中多数不需要全局控制事务，此配置会增加不必要的性能损耗。</b>
 * 
